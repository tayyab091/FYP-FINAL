...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col bg-card/95 backdrop-blur-2xl">
      ...
      {user?.role === 'user' && user.subscription?.plan === 'basic' && (
        <div className="mx-3 mb-3 overflow-hidden rounded-2xl border border-primary/15 bg-[radial-gradient(circle_at_top_right,rgba(34,245,154,.14),transparent_65%),rgba(34,245,154,.035)] p-4">
          <p className="text-sm font-bold text-foreground">Unlock Pro</p>
      ...
      <div className="border-t border-border p-3">
        <div className="mb-2 flex items-center gap-3 rounded-xl px-3 py-2">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-accent text-sm font-black text-primary-foreground shadow-[0_8px_25px_color-mix(in_srgb,var(--primary)_14%,transparent)]">
            {user?.fullName?.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase() || 'U'}
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-foreground">{user?.fullName || 'Account'}</p>
            <p className="truncate text-[11px] capitalize text-muted-foreground">{role.replace('_', ' ')}</p>
          </div>
        </div>
        <Link
          href="/?marketing=1"
          onClick={onNavigate}
          className="mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground lg:hidden"
        >
          Back to site
        </Link>
        <button
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 border-r border-border lg:block">
        <Suspense fallback={<div className="h-full animate-pulse bg-card/95" />}>
          <SidebarContent />
        </Suspense>
      </aside>
      ...
      <header className="fixed inset-x-0 top-0 z-50 flex h-16 items-center justify-between border-b border-border bg-background/92 px-4 backdrop-blur-2xl lg:left-72 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <h1 className="truncate font-heading text-base font-bold tracking-tight text-foreground">{pageTitle}</h1>
          </div>
        </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[70] lg:hidden">
          ...
          />
          <aside className="absolute inset-y-0 left-0 w-[86%] max-w-sm border-r border-border shadow-2xl">
            <button
              type="button"
              onClick={() => setMobileOpen(false)}
              className="absolute right-4 top-5 z-10 rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground"
      ...
      <div className="min-h-screen bg-[radial-gradient(circle_at_70%_-10%,rgba(124,58,237,0.08),transparent_32rem)] pt-16 dark:bg-[radial-gradient(circle_at_70%_-10%,rgba(34,245,154,.055),transparent_32rem)] lg:pl-72">
        {children}
      </div>
    </div>
  )
