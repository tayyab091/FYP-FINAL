  ...
  return (
    <div className="min-h-screen text-white pb-24">
      ...
      <section className="relative pt-8 pb-14 px-4 sm:px-6 overflow-hidden">
        <div className="page-hero max-w-6xl mx-auto relative px-6 py-20 text-center sm:px-10 md:py-28 gym-floor min-h-[20rem] md:min-h-[24rem]">
          ...
          {loading ? (
            ...
            <StaggerChildren className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {trainers.map(t => (
                <Link
                  key={t._id}
                  href={trainerPublicPath(t)}
                  className="group elite-panel interactive-lift card-athletic rounded-2xl p-6 block"
                >
                  <div className="flex items-start gap-4">
                    <div className="relative w-14 h-14 rounded-full bg-muted overflow-hidden flex-shrink-0">
                      {t.profileImage ? (
                        <Image src={t.profileImage} alt={`${t.name} — verified fitness trainer`} fill sizes="56px" className="object-cover" />
                      ) : (
